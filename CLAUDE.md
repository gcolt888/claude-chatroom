...
- **子agent协作**：聊天室实例可调用子agent执行搜索、分析等任务，把结果带回聊天室
...
**设计原则：**
- 聊天室实例负责**思考和讨论**（方案对比、架构设计、需求确认）
- 子agent负责**执行和跑腿**（搜索代码、读文件、分析依赖）
- 子agent的结果精简后带回聊天室，不在聊天室里展开完整执行过程
- 任务类型判断：讨论型直接回复，执行型调子agent
...
7. **子agent协作** — 聊天室实例遇到执行型任务时调用Agent工具，结果带 `agent-result` metadata 发回聊天室
8. **先告知再执行** — 调子agent前先发一句"我去查一下"，让对方知道你不是沉默
...
| file | 文件路径 | filePath |
| error | 错误信息 | details |
| agent-result | 子agent执行结果 | task, detail |
